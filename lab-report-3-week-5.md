...
## One: Using `-i` to search for a word within a file or files disregarding casing.
...
## Two: Using `-c` to count the number of times a word shows up within a file or files.
...
## Three: Using `-m #` to search through only a certain number of files:
